...
| ---- | ---------- | ------- | -------------------- |
| `audio/reference_tone.wav` | Procedurally generated by `waveform/generate_fixtures.py` — 3 s of 440 Hz sine at half scale, 16 kHz mono Int16. | Public domain / CC0 (analytic waveform, no external IP). | No |

## Waveform Parity Fixtures

_(Populated in Task 6.2 per `docs/testing-strategy.md`.)_

Generated files under `__fixtures__/waveform/` are derived from the audio corpus above via
the reference Python generator. Deriving fixtures does not create new copyrighted work in
most jurisdictions, but the source clip's license still governs redistribution.

| File | Derived from | Notes |
| ---- | ------------ | ----- |
| `waveform/reference_tone_windows.bin` | `audio/reference_tone.wav` via `waveform/generate_fixtures.py` | Little-endian float32, 5 × 15,600 samples, normalised by `sample / 32768.0`. Oracle for `WaveformWindowTest` (Android) and `WaveformWindowTests` (iOS). |
| `waveform/generate_fixtures.py` | (dev-only script) | Python 3 + numpy; deterministic (analytic sine). Regenerate both fixtures with `python modules/snoozepulse-audio/__fixtures__/waveform/generate_fixtures.py`. |
